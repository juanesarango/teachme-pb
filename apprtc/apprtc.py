...
def get_room_parameters(request, room_id, client_id, is_initiator):
  ...
  if room_id is not None:
    room_link = request.host_url + '/session/r/' + room_id
...
app = webapp2.WSGIApplication([
    ('/session/', MainPage),
    ('/session/a/', analytics_page.AnalyticsPage),
    ('/session/compute/(\w+)/(\S+)/(\S+)', compute_page.ComputePage),
    ('/session/join/(\w+)', JoinPage),
    ('/session/leave/(\w+)/(\w+)', LeavePage),
    ('/session/message/(\w+)/(\w+)', MessagePage),
    ('/session/params', ParamsPage),
    ('/session/r/(\w+)', RoomPage),
], debug=True)
